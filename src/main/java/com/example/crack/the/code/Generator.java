...
/**
 *
 * this class will generate riddles for the user that can be following defult
 * settings or custom based on the user's settings of: difficulty level length
 * of the code number of clues whether or not to have the possibility of
...
public class Generator {
    ...
    public Riddle generateRiddle() {
        ...
        while (riddle.isSolvable() == false && maxNumberOfClues > clues.size()) {
    ...
    private Clue generateClue() {
        ...
        if (incorrectPlacement > 0) {

            if (correctDigitsList.length == 0) {
                String message = "correct digits list is empty "
                        + " expected: correctDigits: " + correctDigits + " correctPlacement: " + correctPlacement + " incorrectPlacement: " + incorrectPlacement + "\n"
                        + "actual: correctDigits: " + correctDigitsList.length + " correctPlacement: " + correctPlacement + " incorrectPlacement: " + incorrectPlacement + "\n"
        ...
        if (!validator.checkIfComboValid(combination, clue)) {
            ...
            if (validator.getDebugMessages().size() > 0) {
                message += validator.getDebugMessages().get(validator.getDebugMessages().size() - 1);
            }
    ...
    public Integer[] generateCode() {
        ...
        for (int i = 0; i < lengthOfCode; i++) {
            ...
            if (repeatDigits) {
                ...
                while (Arrays.asList(code).contains(random)) {
        ...
        if(!repeatDigits){
            ...
            if(digits.size() != code.length){

                // print stack trace
                System.out.println("Error: the code is not valid - check the Generator class - generateCode method - the par of the code that places the incorrect digits");
                throw new Error("Error: the code is not valid - check the Generator class - generateCode method - the par of the code that places the incorrect digits");
            }
        }
